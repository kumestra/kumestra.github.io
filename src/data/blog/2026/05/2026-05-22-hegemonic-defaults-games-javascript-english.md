...
```mermaid
flowchart LR
  Film --> Genre[Genre: western, noir, musical]
  Film --> Movement[Movement: French New Wave, Italian neorealism]
  Film --> Auteur[Auteur: Kubrick, Tarkovsky, Wong Kar-wai]
  Film --> Form[Form: long-take, montage, slow cinema]
  Film --> Mode[Mode: feature, documentary, experimental]
  Games --> Type[Type: RPG/FPS/MOBA — and that's it]
```
...
```mermaid
flowchart TB
  A[Generic label] --> B[Network effect]
  B --> C[Default-by-inertia]
  C --> D[Loss of internal diversity]
  D --> E[Shrinking imagination]
  E --> F[Next generation can't conceive the alternatives]
```
